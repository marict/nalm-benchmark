...
def test_seed_gives_consistent_output():
    dataset_a = SequentialMnistDataset(operation="count", seed=0).fork(
        seq_length=1, subset="train"
    )

    dataset_b = SequentialMnistDataset(operation="count", seed=0).fork(
        seq_length=1, subset="train"
    )
...
def test_train_is_distinct_from_test():
    dataset_train = SequentialMnistDataset(operation="count", seed=0).fork(
        seq_length=1, subset="train"
    )

    dataset_test = SequentialMnistDataset(operation="count", seed=0).fork(
        seq_length=1, subset="test"
    )
...
def test_count_dataset():
    for seq_length in [1, 10, 100]:
        dataset_labels = SequentialMnistDataset(operation="sum", seed=0).fork(
            seq_length=1, subset="train"
        )

        dataset_count = SequentialMnistDataset(operation="count", seed=0).fork(
            seq_length=seq_length, subset="train"
        )
        ...
        for count_i in range(5):
            ...
            for time_i in range(seq_length):
                ...
                np.testing.assert_almost_equal(
                    x_count[time_i].numpy(), x_label.numpy().reshape(1, 28, 28)
                )
...
def test_sum_dataset():
    for seq_length in [1, 10, 100]:
        ...
        dataset_count = SequentialMnistDataset(operation="sum", seed=0).fork(
            seq_length=seq_length, subset="train"
        )
