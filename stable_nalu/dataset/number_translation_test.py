...
def test_subsets_are_distinct():
    ...
    dataset_train = dataset.fork(subset="train")
    numbers_train = {
        dataset_train[i][1].numpy().item(0) for i in range(len(dataset_train))
    }
    dataset_valid = dataset.fork(subset="valid")
    numbers_valid = {
        dataset_valid[i][1].numpy().item(0) for i in range(len(dataset_valid))
    }
    dataset_test = dataset.fork(subset="test")
    numbers_test = {
        dataset_test[i][1].numpy().item(0) for i in range(len(dataset_test))
    }
