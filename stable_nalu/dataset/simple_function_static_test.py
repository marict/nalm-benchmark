...
def test_solveable_by_linear_algebra():
    ...
    np.testing.assert_almost_equal(
        w_merged_np - w_merged_np_int, np.zeros(100), decimal=4
    )
    # W is either 0, 1, 2
    # NOTE: a different seed might not result in an overlap, thus {2} might
    # not be present.
    assert_equal(set(w_merged_np_int.tolist()), {0, 1, 2})
...
def test_input_range():
    dataset = SimpleFunctionStaticDataset(operation="add", vector_size=10000, seed=0)
    x, t = dataset.fork(input_range=5)[0]
    _, p = scipy.stats.kstest(x, scipy.stats.uniform(loc=0, scale=5).cdf)
    assert p > 0.5
