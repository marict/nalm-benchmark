...
class SequentialSvhnNetwork(ExtendedTorchModule):
    ...
    def __init__(
        ...
        **kwags
        ...
        self.image2label = getattr(torchvision.models, resnet)(num_classes=svhn_outputs)

        if nac_mul == "mnac":
            unit_name = unit_name[0:-3] + "MNAC"
        if self.model_simplification == "none":
            self.recurent_cell = GeneralizedCell(
                svhn_outputs, self.output_size, unit_name, writer=self.writer, **kwags
            )
    ...
    def _forward_trainable_accumulator(self, x):
        ...
        return (torch.stack(l_all).transpose(0, 1), torch.stack(y_all).transpose(0, 1))
    ...
    def extra_repr(self):
        return "unit_name={}, output_size={}".format(self.unit_name, self.output_size)
