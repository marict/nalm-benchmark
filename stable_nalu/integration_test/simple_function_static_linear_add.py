...
def test_linear_add_can_have_zero_loss():
    # Prepear data
    dataset = stable_nalu.dataset.SimpleFunctionStaticDataset(operation="add", seed=0)
    ...
    for i, (x_train, t_train) in zip(range(5), dataset_eval):
        ...
        np.testing.assert_almost_equal(loss.detach().numpy(), 0)
...
def test_linear_add_is_trainable():
    ...
    np.testing.assert_almost_equal(loss.detach().numpy(), 0, decimal=5)
