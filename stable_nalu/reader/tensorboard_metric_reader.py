...
class TensorboardMetricReader:
        ...
        for e in tf.compat.v1.train.summary_iterator(filename):
            ...
            for v in e.summary.value:
                if v.tag == "epoch":
                    ...
                    if current_epoch is not None and len(columns["epoch"]) != len(
                        columns[v.tag]
                    ):
                        columns["epoch"].append(current_epoch)

                elif (
                    v.tag.endswith("W/text_summary")
                    or v.tag.endswith("W_real/text_summary")
                ) and current_logged_step == step:
                    # recursive_weight means verbose flag was set
                    if self.recursive_weight:
                        W = _parse_numpy_str(
                            v.tensor.string_val[0][5:-6].decode("ascii")
                        )
                        ...
                        W = W.reshape(
                            -1
                        )  # flatten first since modules save weights differently e.g. [i,o] or [o,i]
                    ...
                    v.tag.endswith("gate/text_summary")
                    and current_logged_step == step
                    and self.recursive_weight
                ):
                    ...
                    if len(columns["step"]) != len(columns["sparse.error.max"]):
                        ...
                        columns["sparse.error.max"][-1] = max(
                            columns["sparse.error.max"][-1], v.simple_value
                        )

                #######################################################################################################
                # Process sparsity error for the NPU and RealNPU real weight matrix
                elif (
                    v.tag.endswith("W_real/sparsity_error")
                    and current_logged_step == step
                ):
                    # Step changed, update sparse error
                    if len(columns["step"]) != len(columns["sparse.error.max"]):
                ...
                # Process sparsity error for the NPU and RealNPU imaginary weight matrix
                    ...
                    v.tag.endswith("W_im/sparsity_error")
                    and current_logged_step == step
                ):
                    if len(columns["step"]) != len(columns["sparse.error.max"]):
                        assert (
                            "NPU should have already processed a W_real value. "
                ...
                # Process  sparsity error for the MCFC junction
                    ...
                    v.tag.endswith("mcfc_junction/sparsity_error")
                    and current_logged_step == step
                ):
                    # Step changed, update sparse error
                    if len(columns["step"]) != len(columns["sparse.error.max"]):
                ...
                # Process sparsity error for the MCFC out_gate
                    ...
                    v.tag.endswith("mcfc_out_gate/sparsity_error")
                    and current_logged_step == step
                ):
                    if len(columns["step"]) != len(columns["sparse.error.max"]):
                        assert (
                            "MCFC should have already processed a mcfc_junction value. "
                ...
                # Process sparsity error for the (mul)MCFC bias
                    ...
                    v.tag.endswith("mulmcfc_bias/sparsity_error")
                    and current_logged_step == step
                ):
                    if len(columns["step"]) != len(columns["sparse.error.max"]):
                        assert (
                            "MCFC should have already processed a mcfc_junction and mcfc_out_gate value. "
                            "There should already exist a sparsity error in this row"
                        )
                    else:
                        ...
                        columns["sparse.error.max"][-1] = (
                            (columns["sparse.error.max"][-1] * 2) + v.simple_value
                        ) / 3.0

                # Process sparsity error for the MulMCFCSignRealNPU g_hat
                elif (
                    v.tag.endswith("mulmcfcsignrealnpu_g_hat/sparsity_error")
                    and current_logged_step == step
                ):
                    if len(columns["step"]) != len(columns["sparse.error.max"]):
                        assert (
                            "MCFC should have already processed a mcfc_junction, mcfc_out_gate and mulmcfc_bias value. "
                            "There should already exist a sparsity error in this row"
                        )
                    else:
                        ...
                        columns["sparse.error.max"][-1] = (
                            (columns["sparse.error.max"][-1] * 3) + v.simple_value
                        ) / 4.0
    ...
    def __iter__(self):
        ...
        with tqdm(
            total=len(reader), disable=not self.progress_bar
        ) as pbar, multiprocessing.Pool(self.processes) as pool:

            columns_order = None
            for dirname, data in pool.imap_unordered(
                (
                    self._parse_tensorboard_data_for_weights
                    if self.weights_only
                    else self._parse_tensorboard_data
                ),
                reader,
            ):
                pbar.update()
