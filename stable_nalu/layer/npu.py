...
class NPULayer(ExtendedTorchModule):
    ...
    def __init__(self, in_features, out_features, npu_clip="none", **kwargs):
        ...
        if kwargs["regualizer_npu_w"]:
            self._regualizer_W = Regualizer(support="npu", type="W", shape="linear")
        else:
        ...
        return super().regualizer(
            {
                ...
                "W-NPU": (
                    self._regualizer_W([self.W_real, self.W_im])
                    if self.npu_clip == "wig"
                    else self._regualizer_W([self.W_real])
                ),  # the type of reg depends on the clip flag value
            }
        )
    ...
    def forward(self, x):
        ...
        self.writer.add_scalar(
            "W_real/sparsity_error", sparsity_error(self.W_real), verbose_only=False
        )

        self.writer.add_histogram("W_im", self.W_im)
        self.writer.add_tensor("W_im", self.W_im)
        self.writer.add_scalar(
            "W_im/sparsity_error", sparsity_error(self.W_im), verbose_only=False
        )

        g_hat = torch.clamp(self.g, 0.0, 1.0)  # [in]
        self.writer.add_histogram("gate", g_hat)
        self.writer.add_scalar(
            "gate/sparsity_error", sparsity_error(g_hat), verbose_only=False
        )
