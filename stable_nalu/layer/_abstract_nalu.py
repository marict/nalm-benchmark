...
from ..functional.golden_ratio_base import golden_ratio, sigmoid
...
class AbstractNALULayer(ExtendedTorchModule):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if nalu_two_nac and nalu_mul == "mnac":
            self.nac_add = NACOp(
                in_features, out_features, writer=self.writer, name="nac_add", **kwargs
            )
            self.nac_mul = MNACOp(
                in_features, out_features, writer=self.writer, name="nac_mul", **kwargs
            )
        elif nalu_two_nac:
            self.nac_add = NACOp(
                in_features, out_features, writer=self.writer, name="nac_add", **kwargs
            ...
            self.nac_add = NACOp(
                in_features, out_features, writer=self.writer, **kwargs
            )
    ...
    def regualizer(self):
        ...
        if self.nalu_gate == "regualized":
            ...
            regualizers["g"] = torch.sum(
                self.stored_gate_add**2 * (1 - self.stored_gate_add) ** 2
            )
            if self.nalu_two_gate:
                regualizers["g"] += torch.sum(
                    self.stored_gate_mul**2 * (1 - self.stored_gate_mul) ** 2
                )

        if self.nalu_gate == "max-safe":
            regualizers["z"] = torch.mean(
                (1 - self.stored_gate) * torch.relu(1 - self.stored_input)
            )
    ...
    def reset_parameters(self):
        ...
        torch.nn.init.xavier_uniform_(
            self.G_add, gain=torch.nn.init.calculate_gain("sigmoid")
        )
        if self.nalu_two_gate:
            torch.nn.init.xavier_uniform_(
                self.G_mul, gain=torch.nn.init.calculate_gain("sigmoid")
            )
    ...
    def _compute_gate(self, x, G, bias):
        ...
        if self.nalu_gate == "gumbel" or self.nalu_gate == "obs-gumbel":
            ...
            if self.allow_random and self.nalu_gate == "gumbel":
                gumbel = -torch.log(
                    1e-8
                    - torch.log(torch.rand(self.out_features, device=x.device) + 1e-8)
                )
            elif self.allow_random and self.nalu_gate == "obs-gumbel":
                gumbel = -torch.log(
                    1e-8
                    - torch.log(
                        torch.rand(x.size(0), self.out_features, device=x.device) + 1e-8
                    )
                )

            g = torch.sigmoid(
                (torch.nn.functional.linear(x, G, bias) + gumbel) / self.tau
            )
        elif self.nalu_gate == "golden-ratio":
            g = sigmoid(torch.nn.functional.linear(x, G, bias))
        else:
    ...
    def forward(self, x):
        ...
        if self.nalu_mul == "normal":
            ...
            m = torch.sinh(
                self.nac_mul(
                    torch.log(
                        x + (x**2 + 1) ** 0.5 + self.eps
                    )  # torch.asinh(x) does not exist
                )
            )
        elif self.nalu_mul == "mnac":
            m = self.nac_mul(x)
        elif self.nalu_mul == "golden-ratio":
            m = golden_ratio ** (
                self.nac_mul(torch.log(torch.abs(x) + self.eps))
                / torch.log(torch.tensor(golden_ratio))
            )
        else:
