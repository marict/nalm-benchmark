...
class GradientBanditNACLayer(ExtendedTorchModule):
    ...
    def __init__(self, in_features, out_features, **kwargs):
        ...
        # The learning rate
        self.lr = torch.nn.Parameter(
            torch.tensor(1e-3, dtype=torch.float32), requires_grad=False
        )

        # Holds a running mean of the loss
        self.running_mean_beta = torch.nn.Parameter(
            torch.tensor(0.9, dtype=torch.float32), requires_grad=False
        )
    ...
    def optimize(self, loss):
        ...
        self.running_mean_loss.mul_(self.running_mean_beta).add_(
            1 - self.running_mean_beta, loss
        )
        running_mean_loss_debias = self.running_mean_loss / (
            1 - self.running_mean_beta**self.running_mean_iter
        )

        # Convert W sample to a one-hot encoding
        samples_one_hot = torch.zeros(self.out_features, self.in_features, 3).scatter_(
            2, self.sample.view(self.out_features, self.in_features, 1), 1.0
        )

        # Compute update
        pi = torch.nn.functional.softmax(self.W_hat, dim=-1)
        self.W_hat.addcmul_(
            self.lr, running_mean_loss_debias - loss, samples_one_hot - pi
        )
