...
class INALULayer(ExtendedTorchModule):
    ...
    def regualizer(self):
        return super().regualizer(
            {
                "inalu": self._regualizer(
                    [self.W_a_hat, self.M_a_hat, self.W_m_hat, self.M_m_hat, self.g]
                ),
            }
        )
    ...
    def forward(self, x):
        ...
        self.writer.add_scalar(
            "W_a/sparsity_error", sparsity_error_W_a, verbose_only=False
        )
        ...
        self.writer.add_scalar(
            "W_m/sparsity_error", sparsity_error_W_m, verbose_only=False
        )
        self.writer.add_scalar(
            "W/sparsity_error",
            (sparsity_error_W_a + sparsity_error_W_m) / 2.0,
            verbose_only=False,
        )
        ...
        mz = (
            torch.log(
                torch.max(
                    torch.abs(x), torch.full(x.shape, self.eps, dtype=torch.float32)
                )
            )
            @ W_m
        )
        # [B,O]
        m = torch.exp(
            torch.min(mz, torch.full(mz.shape, self.omega, dtype=torch.float32))
        )
        # [B,O] = mnac([B,I], [I,O].t())
        ...
        self.writer.add_scalar(
            "gate/sparsity_error", sparsity_error(g), verbose_only=False
        )
