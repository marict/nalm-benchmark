...
class RealNPULayer(ExtendedTorchModule):
        ...
        if kwargs["regualizer_npu_w"]:
            ...
            if kwargs["realnpu_reg_type"] == "W":
                self._regualizer_W = Regualizer(
                    support="npu", type=kwargs["realnpu_reg_type"], shape="linear"
                )
            # penalise {-1,0,1}
            elif kwargs["realnpu_reg_type"] == "bias":
                self._regualizer_W = Regualizer(
                    support="realnpu", type=kwargs["realnpu_reg_type"], shape="linear"
                )
        else:
    ...
    def forward(self, x):
        ...
        self.writer.add_scalar(
            "W_real/sparsity_error",
            sparsity_error(self.W_real),
        ...
        z = torch.exp(torch.log(r).matmul(self.W_real)) * torch.cos(
            k.matmul(self.W_real)
        )
