...
def binomial_confidence_interval(
    ...
    half_width = (
        z
        * math.sqrt((p_hat * (1 - p_hat) + z**2 / (4 * trials)) / trials)
        / denominator
    )
...
def beta_confidence_interval(
    ...
    if var_scale <= 1:
        ...
        return normal_confidence_interval(
            values, confidence, lower_bound=0.0, upper_bound=1.0
        )
...
if __name__ == "__main__":
    ...
    print(
        f"Mean convergence time: {format_confidence_interval(ci[0], ci[1], mean_time)}"
    )
    ...
    print(
        f"Mean sparsity error: {format_confidence_interval(ci[0], ci[1], mean_sparsity)}"
    )
