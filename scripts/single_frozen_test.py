...
def run_single_experiment(
    ...
    interp_range,
    extrap_range,
    linear_biased_init_G=None,
    freeze_O_selectors_div=None,
    freeze_O_selector_mul=None,
    dag_depth=None,
    use_dense_features=None,
):
    ...
    print(
        f"Testing {operation.upper()} seed {seed} range {interp_range} with {frozen_config}"
    )
...
def main():
    ...
    parser.add_argument(
        "operation", choices=["add", "sub", "mul", "div"], help="Arithmetic operation"
    )
    parser.add_argument("seed", type=int, help="Random seed")
    parser.add_argument(
        "--interp-range", default="[-2,2]", help="Interpolation range (default: [-2,2])"
    )
    parser.add_argument(
        "--extrap-range",
        default="[[-6,-2],[2,6]]",
        help="Extrapolation range (default: [[-6,-2],[2,6]])",
    )
    parser.add_argument("--output", help="Output JSON file path")

    # DAG-specific parameters
    parser.add_argument(
        "--linear-biased-init-G",
        type=lambda x: x.lower() in ["true", "1", "yes"],
        help="Enable linear biased G initialization",
    )
    parser.add_argument(
        "--freeze-O-selectors-div",
        type=lambda x: x.lower() in ["true", "1", "yes"],
        help="Freeze O selectors for division pattern [1,-1,0,...]",
    )
    parser.add_argument(
        "--freeze-O-selector-mul",
        type=lambda x: x.lower() in ["true", "1", "yes"],
