...
def main():
    ...
    print(f"MUL (frozen mul selectors): {mul_success_rate:.1%} ({sum(mul_results)}/{len(mul_results)})")
    print(f"DIV (frozen div selectors): {div_success_rate:.1%} ({sum(div_results)}/{len(div_results)})")
