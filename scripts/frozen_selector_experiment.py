...
def run_single_experiment(
    operation: str, seed: int, interp_range, extrap_range, frozen_config: dict
):
        ...
        for range_key, range_results in by_range.items():
            ...
            range_analysis[range_key] = {
                ...
                "avg_grok_step": (
                    ...
                    if any(r["grok_step"] for r in range_results)
                    else None
                ),
            }
...
def print_summary(analysis):
    ...
    print(
        f"Overall grok rate: {analysis['grokked_runs']/analysis['total_experiments']:.1%}"
    )
    ...
    for op, stats in analysis["operations"].items():
        print(f"\n{op.upper()}:")
        print(
            f"  Success rate: {stats['success_rate']:.1%} ({stats['successful_runs']}/{stats['total_tests']})"
        )
        print(
            f"  Grok rate: {stats['grok_rate']:.1%} ({stats['grokked_runs']}/{stats['total_tests']})"
        )
        ...
        for range_key, range_stats in stats["by_range"].items():
            print(
                f"    {range_key}: {range_stats['grok_rate']:.1%} ({range_stats['grokked']}/{range_stats['total_tests']})"
            )
...
def main():
    ...
    parser.add_argument(
        "--output-dir", type=str, default="experiment_results", help="Output directory"
    )
    ...
    for operation in ["mul", "add"]:
        ...
        for seed in TEST_SEEDS:
            for interp_range, extrap_range in PAPER_RANGES:
                experiments.append(
                    (operation, seed, interp_range, extrap_range, frozen_config)
                )
    ...
    print(
        f"Operations: mul/add (with freeze_O_selector_mul), sub/div (with freeze_O_selectors_div)"
    )
    ...
    with ProcessPoolExecutor(max_workers=args.max_workers) as executor:
        ...
        for future in as_completed(future_to_exp):
            ...
            if result["grokked"]:
                print(
                    f"✅ {result['operation']} seed {result['seed']} range {result['interp_range']}: GROKKED at step {result['grok_step']}"
                )
            else:
                print(
                    f"❌ {result['operation']} seed {result['seed']} range {result['interp_range']}: Failed (loss: {result['final_inter_loss']:.2e})"
                )
