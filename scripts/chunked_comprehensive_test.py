...
Features:
- Individual test progress bars with tqdm showing training iterations and loss values
- Concurrent execution support for faster completion
- Incremental result saving with caching for interrupted runs
- Comprehensive success rate analysis and summary tables
"""
...
def run_operation_chunk(target_operation, target_range_idx=None):
    ...
    if target_range_idx is not None:
        ...
        print(
            f"🔸 Testing {target_operation.upper()} on range {target_range_idx}: {TEST_RANGES[target_range_idx][2]}"
        )
    else:
        ranges_to_test = TEST_RANGES
        print(f"🔸 Testing all ranges for {target_operation.upper()}")

    for range_idx, (interp_range, extrap_range, range_name) in enumerate(
        ranges_to_test
    ):
        if target_range_idx is not None:
...
def run_operation_chunk_concurrent(
    target_operation, target_range_idx=None, max_workers=4
):
    ...
    if target_range_idx is not None:
        ranges_to_test = [TEST_RANGES[target_range_idx]]
        print(
            f"🔸 Testing {target_operation.upper()} on range {target_range_idx}: {TEST_RANGES[target_range_idx][2]} (CONCURRENT)"
        )
    else:
        ranges_to_test = TEST_RANGES
        print(
            f"🔸 Testing all ranges for {target_operation.upper()} (CONCURRENT - {max_workers} workers)"
        )
    ...
    with ProcessPoolExecutor(max_workers=max_workers) as executor:
        # Submit batches
        future_to_batch = {
            executor.submit(run_test_batch, batch): batch for batch in test_batches
        }
        ...
        for future in as_completed(future_to_batch):
            try:
                ...
                print(
                    f"  Progress: {completed_count}/{total_tests} ({progress_pct:.1f}%) - Latest batch: ",
                    end="",
                )
...
def generate_final_table():
    ...
    overall_success_rate = (
        total_grokked / total_experiments * 100 if total_experiments > 0 else 0
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--operation", choices=OPERATIONS, help="Run specific operation only"
    )
    parser.add_argument(
        "--range-idx", type=int, help="Run specific range index only (0-8)"
    )
    parser.add_argument(
        "--table-only",
        action="store_true",
        help="Just generate table from existing results",
    )
    parser.add_argument(
        "--concurrent", action="store_true", help="Run tests concurrently (faster)"
    )
    parser.add_argument(
        "--workers",
        type=int,
    ...
    if args.table_only:
        ...
        print(
            "  python chunked_comprehensive_test.py --operation div --concurrent --workers 6"
        )
        print(
            "  python chunked_comprehensive_test.py --operation add --range-idx 2 --concurrent"
        )
        print()
