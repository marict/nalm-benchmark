...
def _parse_tags(raw: str | None) -> List[str]:
    if not raw:
        return []
    # Strict: require a JSON array of strings; error if not provided correctly
    try:
...
def _init_wandb() -> wandb.sdk.wandb_run.Run:
    ...
    try:
        ...
        # Surface a clear, hard failure with context from environment
        context = {
            "project": project,
            "entity": entity,
...
run = _init_wandb()
# Expose the actual wandb module under a guarded namespace so callsites use
# `wandb.wrapper.log(...)`, `wandb.wrapper.run`, etc., ensuring env checks ran.
wrapper = wandb
