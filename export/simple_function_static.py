...
def matcher(tag):
    return (
        tag
        in ["metric/valid/interpolation", "metric/test/extrapolation", "W/text_summary"]
        or tag.endswith("nalu/gate/mean")
        and args.export_gate
        if args.verbose
        else tag in ["metric/valid/interpolation", "metric/test/extrapolation"]
        or tag.endswith("nalu/gate/mean")
        and args.export_gate
    )
