...
def test_sparsity_during_simulated_training():
    ...
    x_train = torch.tensor([[2.0, 3.0], [1.0, 4.0], [-1.0, 2.0], [0.5, 6.0]])
    ...
    if len(sparsity_values) >= 2:
        ...
        if sparsity_range > 1e-6:
            print(
                "  ✅ Sparsity values are changing - calculation appears to be based on actual weights"
            )
        else:
            print(
                "  ⚠️  Sparsity values are not changing - may be hardcoded or weights not updating"
            )
    ...
    print(
        f"  Perfect discrete [1.0, 1.0]: sparsity = {discrete_sparsity:.6f} (should be 0.0)"
    )
...
def test_dag_depth_constraint():
    ...
    for depth in [1, 2, 3]:
        try:
            ...
            sparsity = layer.calculate_sparsity_error("mul")
            print(
                f"  dag_depth={depth}: sparsity = {sparsity:.6f} (should only work for depth=1)"
            )
...
if __name__ == "__main__":
    ...
    if len(sparsity_values) > 0:
        ...
        if sparsity_range > 1e-6:
            ...
            print(
                "❌ FAILED: Sparsity values appear to be hardcoded or weights not changing"
            )
    else:
